...
// Helper function to save PDF using Capacitor Filesystem
async function savePdfToFile(fileName: string, dataAsBase64: string) {
  try {
    try {
      ...
      if (permStatus.publicStorage !== 'granted') {
        ...
        if (request.publicStorage !== 'granted') {
          alert('Permissão de armazenamento necessária para salvar o PDF.');
          return;
        }
      }
    } catch (e) {
    ...
    alert(`'${fileName}' foi salvo na sua pasta de Documentos.`);
  } catch (error) {
...
export default function PlayerStatsSpreadsheet({ actions, teamAName, teamBName }: PlayerStatsSpreadsheetProps) {
  ...
  const exportTeamToPDF = async (team: "A" | "B") => {
    ...
    await savePdfToFile(`${teamName}_stats.pdf`, base64Data);
  }
  ...
  const exportToPDF = async () => {
    ...
    const pdfOutput = doc.output('datauristring');
    const base64Data = pdfOutput.split(',')[1];
    await savePdfToFile(`${teamAName}_vs_${teamBName}_stats.pdf`, base64Data);
  }
