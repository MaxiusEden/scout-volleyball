...
export default function EightFaceDataEntry({
  ...
  const [actionData, setActionData] = useState<Partial<MatchAction>>({
    ...
    servingPlayer: 0,
    serveQuality: undefined,
    serveZone: undefined,
    passingQuality: undefined,
    passingPlayer: 0,
    attackingTeam: undefined,
    attackPosition: undefined,
    resultComplemento: undefined,
    actionPlayer: 0,
    defensivePlayer: 0,
    transitionType: "k1",
  ...
  const handleFaceComplete = (value: string | number) => {
    ...
    switch (currentFace) {
      case 1:
        newData.servingTeam = value as "A" | "B"
        break
      case 2:
        newData.servingPlayer = value as number
        break
      case 3:
        newData.serveQuality = value as "+" | "-" | "ka"
        if (value === "-" || value === "ka") {
        ...
        break
      case 4:
        newData.serveZone = value as "7.5" | "8.6" | "9.1"
        break
      case 5:
        newData.passingQuality = value as "A" | "B" | "C" | "D"
        if (value === "D") {
      ...
      case 6:
        ...
        return
      case 7:
        newData.attackingTeam = value as "A" | "B"
        break
      case 8:
        newData.attackPosition = value as "O" | "M" | "P" | "F" | "S"
        break
      case 9:
        newData.resultComplemento = value as "#" | "!" | "+" | "D" | "$" | "%"
        if (value === "D") {
          setActionData(newData)
      ...
      case 10:
        ...
        break
      case 10.5:
        newData.blockingPosition = value as "O" | "M" | "P" | "FS"
        setActionData(newData)
        setCurrentFace(11)
        return
      case 11:
        ...
        return
      case 12:
        newData.transitionType = value as "k1" | "k2" | "k3"
        const action: MatchAction = {
          id: Math.random().toString(),
          timestamp: Date.now(),
          servingTeam: actionData.servingTeam as "A" | "B",
          ...
          blockingPosition: actionData.resultComplemento === "+" ? (actionData.blockingPosition as "O" | "M" | "P" | "FS") : undefined,
      ...
      <div className="w-full max-w-2xl mx-auto mb-4 p-4 bg-gradient-to-r from-red-600 to-blue-600 rounded-lg shadow-lg">
        <div className="flex justify-between items-center text-black">
          <div className="text-center flex-1">
            <h2 className="text-xl sm:text-2xl font-bold truncate">{teamAName}</h2>
            <p className="text-3xl sm:text-4xl font-bold">{teamAScore}</p>
          </div>
          ...
          <div className="text-center flex-1">
            <h2 className="text-xl sm:text-2xl font-bold truncate">{teamBName}</h2>
            <p className="text-3xl sm:text-4xl font-bold">{teamBScore}</p>
          </div>
        </div>
      </div>

      <Card className="w-full max-w-2xl mx-auto p-4 sm:p-8 bg-white shadow-2xl text-black">
        <div className="mb-4 sm:mb-8 text-center sm:text-left">
          <h1 className="text-2xl sm:text-3xl font-bold text-black mb-2">Coleta de Dados</h1>
        </div>

        <div className="mb-6 p-2 sm:p-4 bg-blue-50 text-black rounded-lg border border-blue-200">
          ...
          {currentFace === 10.5 && (
            <div>
              <h2 className="text-lg font-semibold mb-4 text-black">Posição do Bloqueio</h2>
              <div className="grid grid-cols-3 gap-2">
                <Button onClick={() => handleFaceComplete("P")} className="h-12 text-lg bg-white border border-slate-300 text-black hover:bg-slate-100" variant="outline">
                  P - Ponta
                </Button>
                <Button onClick={() => handleFaceComplete("M")} className="h-12 text-lg bg-white border border-slate-300 text-black hover:bg-slate-100" variant="outline">
                  M - Meio
                </Button>
                <Button onClick={() => handleFaceComplete("O")} className="h-12 text-lg bg-white border border-slate-300 text-black hover:bg-slate-100" variant="outline">
                  O - Oposto
                </Button>
              </div>
            </div>
        ...
        <div className="flex justify-between mt-4">
          <Button
            ...
            className="w-32 h-12 text-lg bg-white text-black border-slate-300 hover:bg-slate-100 shadow-sm disabled:opacity-50"
          >
            Anterior
          </Button>
          <Button onClick={() => setCurrentFace(1)} variant="outline" className="w-32 h-12 text-lg bg-white text-black border-slate-300 shadow-sm hover:bg-slate-100">
            Limpar
          </Button>
        </div>
      </Card>
